...
class RemoveItemActorIntegrationTest : AutoCloseKoinTest() {

    private val itemDao: ItemDao by inject()
    private val removeItemsActor: RemoveItemActor by inject()
    ...
    fun `should return a list of items`() = runBlocking {
        ...
        given(itemDao.deleteItem(repositoryItem)).willReturn(Unit)

        val domainItem = DomainItem(id = 1, name = "name1", color = "color1")
        val response: TodoResponse.CommandSuccess? = getFlowForCommandSuccess(removeItemsActor.run(domainItem))

        Assertions.assertNotNull(response)
        Mockito.verify(itemDao).deleteItem(repositoryItem)
    }
